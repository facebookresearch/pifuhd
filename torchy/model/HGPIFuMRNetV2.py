...
class HGPIFuMRNetV2(BasePIFuNet):
    '''
    ...
    def __init__(self, 
        ...
        super(HGPIFuMRNetV2, self).__init__(
    ...
    def filter_local(self, images, rect=None):
        ...
        #     img = 255*(0.5*images[i][:3].permute(1,2,0).detach().cpu().numpy()+0.5)
        #     nmlb = 255*(0.5*images[i][3:6].permute(1,2,0).detach().cpu().numpy()+0.5)
        #     nmlf = 255*(0.5*images[i][6:].permute(1,2,0).detach().cpu().numpy()+0.5)
        #     print(img.shape, nmlb.shape, nmlf.shape)
        #     img = np.concatenate([img,nmlb,nmlf], 1)
