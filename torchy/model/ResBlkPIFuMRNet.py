...
class ResBlkPIFuMRNet(BasePIFuNet):
    '''
    ...
    def __init__(self, 
        ...
        super(ResBlkPIFuMRNet, self).__init__(
    ...
    def filter_local(self, images, rect=None):
        ...
        if len(nmls):
            nmls = nn.Upsample(size=(self.opt.loadSizeBig,self.opt.loadSizeBig), mode='bilinear')(torch.cat(nmls,1))
    ...
    def query(self, points, calib_local, calib_global=None, transforms=None, labels=None):
        ...
        for i in range(B):
            ...
            if self.spatial_enc is not None:
                z_feat = torch.cat([z_feat, self.spatial_enc(xyz, calibs=calib_local[:,i])], 1)
        ...
        # save_samples_truncted_prob('tmp/gt.ply', points[0,0].t().detach().cpu().numpy(), self.labels[0].t().detach().cpu().numpy())
        # save_samples_truncted_prob('tmp/pred.ply', points[0,0].t().detach().cpu().numpy(), self.preds_low[0,0].t().detach().cpu().numpy())
        # import cv2
        # imgss = []
        # imgss_g = []
        # for i in range(calib_local.size(1)):
        #     xyz = self.projection(points[:,i], calib_local[:,i], transforms)
        #     pts = self.opt.loadSizeLocal*(0.5 * xyz[:, :2] + 0.5).detach().cpu().numpy()

        #     xyz = self.projection(points[:,i], calib_global, transforms)
        #     pts_g = self.opt.loadSize*(0.5 * xyz[:, :2] + 0.5).detach().cpu().numpy()
        #     imgs = []
        #     imgs_g = []
        #     for j in range(calib_local.size(0)):
        #         img = 255.0*(0.5*self.imgss_l[j,i].permute(1,2,0).detach().cpu().numpy()+0.5)[:,:,:3]
        #         for p in pts[j].T:
        #             img = cv2.circle(img, (int(p[0]),int(p[1])), 2, (0,255.0,0), -1)      
        #         imgs.append(img)
        #         img = 255.0*(0.5*self.imgss_g[j].permute(1,2,0).detach().cpu().numpy()+0.5)[:,:,:3]
        #         for p in pts_g[j].T:
        #             img = cv2.circle(img, (int(p[0]),int(p[1])), 2, (0,255.0,0), -1)
        #         imgs_g.append(img)
        ...
        #     imgss.append(imgs)
        #     imgs_g = np.concatenate(imgs_g, 0)
        #     imgss_g.append(imgs_g)
        # imgss = np.concatenate(imgss, 1)
        # cv2.imwrite('tmp/local.png', imgss)
        # imgss_g = np.concatenate(imgss_g, 1)
        # cv2.imwrite('tmp/global.png', imgss_g)
        # exit()
