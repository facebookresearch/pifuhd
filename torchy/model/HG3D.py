...
class HGFilter3D(nn.Module):
    def __init__(self, in_ch, stack, depth, last_ch, norm='batch', down_type='conv64', use_sigmoid=True):
        ...
        self.conv1 = nn.Conv3d(in_ch, 32, kernel_size=7, stride=2, padding=3)
        ...
        if self.down_type == 'conv32':
            self.conv2 = ConvBlock(32, 32, self.norm)
            self.down_conv2 = nn.Conv3d(32, 64, kernel_size=3, stride=2, padding=1)
        elif self.down_type == 'conv64':
            self.conv2 = ConvBlock(64, 64, self.norm)
            self.down_conv2 = nn.Conv3d(64, 64, kernel_size=3, stride=2, padding=1)
        elif self.down_type == 'ave_pool':
            self.conv2 = ConvBlock(32, 64, self.norm)
        
        self.conv3 = ConvBlock(64, 64, self.norm)
        self.conv4 = ConvBlock(64, 128, self.norm)
        
        # start stacking
        for stack in range(self.n_stack):
            self.add_module('m' + str(stack), HourGlass3D(self.depth, 128, self.norm))

            self.add_module('top_m_' + str(stack), ConvBlock(128, 128, self.norm))
            self.add_module('conv_last' + str(stack),
                            nn.Conv3d(128, 128, kernel_size=1, stride=1, padding=0))
            if self.norm == 'batch':
                self.add_module('bn_end' + str(stack), nn.BatchNorm3d(128))
            elif self.norm == 'group':
                self.add_module('bn_end' + str(stack), nn.GroupNorm(32, 128))
            
            self.add_module('l' + str(stack),
                            nn.Conv3d(128, self.last_ch, kernel_size=1, stride=1, padding=0))
            
            if stack < self.n_stack - 1:
                self.add_module(
                    'bl' + str(stack), nn.Conv3d(128, 128, kernel_size=1, stride=1, padding=0))
                self.add_module(
                    'al' + str(stack), nn.Conv3d(self.last_ch, 128, kernel_size=1, stride=1, padding=0))

    def forward(self, x):
        x = F.relu(self.bn1(self.conv1(x)), True)

        if self.down_type == 'ave_pool':
            x = F.avg_pool3d(self.conv2(x), 2, stride=2)
        elif self.down_type == ['conv32', 'conv64']:
            x = self.conv2(x)
            x = self.down_conv2(x)
        else:
